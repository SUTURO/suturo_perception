...
public:
    ...
    TyErrorId initialize(AnnotatorContext& ctx)
    {
        ...
        if (ctx.isParameterDefined("defaultRegions"))
        {
            outInfo("initialize..defaultregions");
            ctx.extractValue("defaultRegions", temp);
            for (auto s : temp)
            {
                outInfo("s" << *s);
...
private:
    TyErrorId processWithLock(CAS& tcas, ResultSpecification const& res_spec)
    {
        ...
        for (size_t i = 0; i < semantic_map_items_.size(); ++i)
        {
            ...
            obj.name(item.name);
            obj.typeName(item.reference_frame); //TODO hack
            ...
            if (it != semantic_map_items_.end())
            {
                ...
                if (frustumCulling(item) || !frustum_culling_)
                {
                    ...
                    if (item.has_plane_equations)
                    {
                        ...
                        supp_plane.source("SuturoRegionFilter");
    ...
    void filterRegion(const SemanticMapItem& region)
    {
        ...
        if(region.reference_frame == "map") {
            ...
            outInfo(std::to_string( camToWorld.getOrigin().x()) + "+" + std::to_string(camToWorld.getOrigin().y()) + "+" + std::to_string(camToWorld.getOrigin().z()));
            outInfo(std::to_string( transform.getOrigin().x()) + "+" + std::to_string(transform.getOrigin().y()) + "+" + std::to_string(transform.getOrigin().z()));
            outInfo("Map als Frame");
        } else {
            transform = region.transform.inverse() * camToWorld;
            outInfo(std::to_string( camToWorld.getOrigin().x()) + "+" + std::to_string(camToWorld.getOrigin().y()) + "+" + std::to_string(camToWorld.getOrigin().z()));
            outInfo(std::to_string( transform.getOrigin().x()) + "+" + std::to_string(transform.getOrigin().y()) + "+" + std::to_string(transform.getOrigin().z()));
            outInfo("andere Frame");
        }
