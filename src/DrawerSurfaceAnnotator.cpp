...
class DrawerSurfaceAnnotator : public DrawingAnnotator
{
private:
    enum
    {
...
public:
    ...
    DrawerSurfaceAnnotator(): DrawingAnnotator(__func__)
            , mode(BOARD)
...
private:
    ...
    void estimateFromPCL(CAS& tcas, rs::Scene& scene)
    {
        ...
        if (process_cloud(plane_coefficients))
        {
            ...
            //add plane to scene clusters
            rs::ObjectHypothesis uimaCluster = rs::create<rs::ObjectHypothesis>(tcas);
            rs::ReferenceClusterPoints rcp = rs::create<rs::ReferenceClusterPoints>(tcas);
            rs::PointIndices uimaIndices = rs::conversion::to(tcas, *plane_inliers);
            ...
            imageRoi.mask_hires(rs::conversion::to(tcas, mask));
            ...
            uimaCluster.source.set("RANSAC");
            scene.identifiables.append(uimaCluster);

            std::vector<rs::ObjectHypothesis> clusters;
            scene.identifiables.filterOverAllSubtypes(clusters);
            outInfo("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" << clusters[0].id.get());
        }
        else
        {
            outInfo("No plane found in the cloud");
...
MAKE_AE(DrawerSurfaceAnnotator)
